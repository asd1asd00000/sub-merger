...
func handleSettings(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		settings := models.SystemSettings{
			...
			TutorialsURL:     strings.TrimSpace(r.FormValue("tutorials_url")),     // فیلد جدید فرم
			AnnouncementsURL: strings.TrimSpace(r.FormValue("announcements_url")), // فیلد جدید فرم
		}
		
		db.SaveSettings(settings)
		
		go db.TriggerInitialTelegramSync(settings)
